    ...
    <div className="overflow-hidden py-8">
      ...
      >
        {doubled.map((img, i) => (
          <motion.div
            key={i}
            className="flex-shrink-0 w-64 h-80 rounded-2xl overflow-hidden cursor-pointer card-3d"
...
const Index = () => {
  ...
  return (
    <div className="min-h-screen bg-background pb-16 md:pb-0">
      ...
      {/* Top Nav */}
      <header>
      <nav className="sticky top-0 z-40 backdrop-blur-md border-b border-border" style={{ background: "linear-gradient(135deg, hsl(37 30% 93%), hsl(36 28% 91%))", borderBottom: "2px solid hsl(38 50% 42% / 0.3)", boxShadow: "0 2px 12px hsl(25 20% 20% / 0.08)" }}>
        <div className="container mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Button variant="outline" size="sm" onClick={() => navigate("/track-order")} className="rounded-full font-sans gap-1 hidden sm:flex btn-3d">
              ...
              user ? (
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="outline" size="sm" className="rounded-full font-sans gap-2 btn-3d">
                ...
                <div className="flex gap-2">
                  ...
                  <Button size="sm" onClick={() => navigate("/register")} className="rounded-full font-sans btn-3d bg-primary hover:bg-primary/90">Register</Button>
                </div>
              )
            )}
          </div>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.98 }}>
                <Button size="lg" onClick={handleOrderClick}
                  className="text-base md:text-lg px-8 md:px-10 py-6 rounded-full font-sans font-semibold btn-3d bg-primary hover:bg-primary/90"
              ...
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.98 }}>
                <Button size="lg" variant="outline" onClick={() => navigate("/track-order")}
                  className="text-base md:text-lg px-8 py-6 rounded-full font-sans font-semibold border-primary/30 hover:bg-primary/10 btn-3d">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
          {[
            ...
            <motion.div key={item.step} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.15 }} whileHover={{ y: -8, scale: 1.02 }}
              ...
              <Card className="text-center border-none card-3d bg-card" style={{ boxShadow: "var(--shadow-card)" }}>
                <CardContent className="pt-8 pb-6 px-6">
                  <div className="w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-4" style={{ background: "linear-gradient(135deg, hsl(38 50% 42% / 0.15), hsl(30 45% 50% / 0.1))" }}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-6 max-w-4xl mx-auto">
            {styles.map((style, i) => (
              <motion.div key={style.name} initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.08 }} whileHover={{ scale: 1.05, rotateZ: 1 }}
                ...
                <Card className="group cursor-pointer border border-border/50 hover:border-primary/30 transition-all card-3d bg-card">
                  <CardContent className="p-4 md:p-6 text-center">
                    <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl flex items-center justify-center mx-auto mb-3 group-hover:bg-primary/20 transition-colors" style={{ background: "linear-gradient(135deg, hsl(38 50% 42% / 0.15), hsl(30 45% 50% / 0.1))" }}>
      ...
      <section className="container mx-auto px-4 py-16 md:py-20">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center max-w-2xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.98 }}>
              <Button size="lg" onClick={handleOrderClick} className="rounded-full px-10 py-6 text-base md:text-lg font-sans font-semibold btn-3d bg-primary hover:bg-primary/90">
                Start Your Order <ArrowRight className="w-5 h-5 ml-2" />
              </Button>
            </motion.div>
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.98 }}>
              <Button size="lg" variant="outline" onClick={() => navigate("/track-order")} className="rounded-full px-10 py-6 text-base md:text-lg font-sans font-semibold border-primary/30 hover:bg-primary/10 btn-3d">
        ...
        <div className="container mx-auto px-4 text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <a href={`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent("Hi! I have a question about caricatures.")}`} target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-2 bg-[#25D366] text-white rounded-full py-3 px-6 font-sans font-medium hover:opacity-90 transition-opacity btn-3d">
              <MessageCircle className="w-5 h-5" /> Chat on WhatsApp
            </a>
            <a href={INSTAGRAM_URL} target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-2 bg-gradient-to-r from-[#833AB4] via-[#FD1D1D] to-[#F77737] text-white rounded-full py-3 px-6 font-sans font-medium hover:opacity-90 transition-opacity btn-3d">
      ...
      {/* Footer */}
      <footer className="py-8 border-t border-border" style={{ background: "linear-gradient(135deg, hsl(37 30% 93%), hsl(36 28% 91%))", boxShadow: "0 -2px 12px hsl(25 20% 20% / 0.05)" }}>
        <div className="container mx-auto px-4">
