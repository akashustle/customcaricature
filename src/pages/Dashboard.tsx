...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "profiles", filter: `user_id=eq.${user.id}` }, () => fetchProfile(user.id, { force: true }))
            ...
            return ordered.filter(t => (tabsAvailable as any)[t.key]).map((item) => {
              ...
              return (
                <button
                  ...
                  className={`relative flex flex-col items-center justify-center gap-1 min-w-[64px] h-14 px-3 py-1.5 rounded-2xl transition-all flex-shrink-0 ${
                    isActive ? "bg-primary text-primary-foreground" : "text-muted-foreground"
                  }`}
                >
                  <item.icon className="w-5 h-5" strokeWidth={isActive ? 2.4 : 1.8} />
                  <span className={`text-[10.5px] leading-none font-sans ${isActive ? "font-semibold" : "font-medium"}`}>{item.label}</span>
                </button>
              );
            });
          })()}
