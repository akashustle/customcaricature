...
const AdminAIAssistant = ({ embedded = false, hideTrigger = false, open: openProp, onOpenChange }: AdminAIAssistantProps = {}) => {
  ...
  const HeaderBlock = (
    <div className={embedded ? "p-4 border-b border-border/40 bg-gradient-to-r from-primary/5 to-accent/5 rounded-t-2xl" : ""}>
      <div className="flex items-center gap-2 font-sans">
        ...
        <div className="flex-1 text-left">
          ...
          <div className="text-[10px] font-normal text-muted-foreground">Toggles · Pricing · Reports · Broadcasts · Navigate</div>
        </div>
        <button
    ...
    <>
        ...
        {messages.length === 0 && (
          <div className="space-y-3">
            ...
            <p className="text-[10px] text-muted-foreground mt-4 font-sans leading-relaxed">
              💡 Try: "Open FAQs and add a new FAQ", "Go to Homepage", "Turn on maintenance and broadcast 'Back at 5pm'".
            </p>
          </div>
        )}
        {messages.map((m, i) => (
      ...
      {showDebug && (
        <div className="border-t border-amber-500/30 bg-amber-500/5 max-h-[40vh] overflow-y-auto">
          ...
          {!debug ? (
            <p className="p-3 text-[11px] text-muted-foreground font-sans">No calls yet.</p>
          ) : (
      ...
      <div className="border-t border-border/40 p-3 bg-card/50">
        <div className="flex gap-2 items-end">
          <Textarea
            ...
            placeholder="e.g. 'Open FAQs', 'Change couple caricature to ₹9999', 'Broadcast: back at 5pm'"
            rows={2}
        ...
        <p className="text-[10px] text-muted-foreground mt-1.5 text-center font-sans">Powered by Lovable AI · Full autonomous admin control</p>
      </div>
    </>
  );
  ...
  if (embedded) {
    return (
      <div className="flex flex-col h-[calc(100vh-220px)] min-h-[500px] rounded-2xl border border-border/40 bg-card/50 overflow-hidden">
        {HeaderBlock}
        {Body}
      </div>
    );
    ...
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent side="right" className="w-full sm:max-w-md p-0 flex flex-col">
        <SheetHeader className="p-4 border-b border-border/40 bg-gradient-to-r from-primary/5 to-accent/5">
          <SheetTitle asChild><div>{HeaderBlock}</div></SheetTitle>
        </SheetHeader>
        {Body}
      </SheetContent>
    </Sheet>
