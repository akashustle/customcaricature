        ...
        {items.map((item) => {
          ...
          return (
            <button
              key={item.path + item.label}
              onClick={() => navigate(item.path)}
              className={`relative flex flex-col items-center justify-center gap-1 min-w-[64px] h-14 px-3 py-1.5 rounded-2xl transition-colors flex-shrink-0 active:scale-95 ${
