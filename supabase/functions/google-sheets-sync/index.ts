...
const SUMMARY_COL_COUNT = SUMMARY_HEADERS.length; // 20
...
function formatSheetRow(event: any, artistNames: string[]): any[] {
  ...
  return [
    ...
    negotiatedTotal,
    negotiatedAdvance,
    `=IF(E{ROW}-F{ROW}<0,0,E{ROW}-F{ROW})`,
    fullAddress,
    event.client_name || "",
    event.client_mobile || "",
    event.client_email || "",
...
function formatSummaryRow(event: any, artistNames: string[], srNo: number): any[] {
  ...
  if (startTime && endTime) {
    ...
    const hour = new Date(event.event_date + "T12:00:00").getHours();
    timeStr = hour < 12 ? "Morning" : hour < 17 ? "Afternoon" : "Evening";
  }
...
async function rebuildSummarySheet(accessToken: string, spreadsheetId: string, tabs: any[], supabase: any) {
  // Ensure Summary tab exists at index 0
  ...
  // Fetch ALL events from DB ordered by event_date
  ...
  // Fetch artist names for all events in batch
  ...
  for (const event of (events || [])) {
    const names = artistMap[event.id] || [];
    allRows.push(formatSummaryRow(event, names, srNo));
    srNo++;
  }
  ...
  if (allRows.length > 0) {
    const lastCol = "T";
    await updateSheet(accessToken, spreadsheetId, `'${SUMMARY_TAB_NAME}'!A1:${lastCol}${allRows.length}`, allRows);
  }

  // Apply formatting
  const sid = summaryTab.sheetId;
  const formatReqs: any[] = [
    // Header style - light blue
    {
      repeatCell: {
        ...
        cell: {
          userEnteredFormat: {
            ...
            textFormat: { bold: true, foregroundColor: { red: 0.1, green: 0.15, blue: 0.3 }, fontSize: 10, fontFamily: "Google Sans" },
    ...
    // Currency format for G, H, I (Total, Paid, Remaining) - cols 6,7,8
    {
      repeatCell: {
  ...
  formatReqs.push({
    addBanding: {
      bandedRange: {
        ...
        rowProperties: {
          headerColor: { red: 0.82, green: 0.90, blue: 0.97 },
  ...
  // Conditional: Pending = red, Paid = green, Partial = amber on Payment Status col (index 15)
  formatReqs.push(
    {
      addConditionalFormatRule: {
        rule: {
          ranges: [{ sheetId: sid, startRowIndex: 1, endRowIndex: 1000, startColumnIndex: 15, endColumnIndex: 16 }],
          booleanRule: {
...
async function setupSheets(accessToken: string, spreadsheetId: string, supabase?: any) {
  ...
  if (tabsToCreate.length > 0) {
    const requests: any[] = tabsToCreate.map((title, i) => ({
      addSheet: { properties: { title, index: i + 1 } }, // +1 to leave index 0 for summary
    }));
    ...
    for (const tabName of tabsToCreate) {
      ...
      formatRequests.push({
        setDataValidation: {
          ...
          rule: { condition: { type: "ONE_OF_LIST", values: [{ userEnteredValue: "Pending" }, { userEnteredValue: "Partial" }, { userEnteredValue: "Paid" }] }, showCustomUi: true, strict: false },
        },
      });
      formatRequests.push({
        setDataValidation: {
          range: { sheetId: sid, startRowIndex: 1, endRowIndex: 200, startColumnIndex: 15, endColumnIndex: 16 },
          rule: { condition: { type: "ONE_OF_LIST", values: [{ userEnteredValue: "New" }, { userEnteredValue: "Confirmed" }, { userEnteredValue: "Completed" }, { userEnteredValue: "Cancelled" }] }, showCustomUi: true, strict: false },
        },
      });
      formatRequests.push({
        setDataValidation: {
          range: { sheetId: sid, startRowIndex: 1, endRowIndex: 200, startColumnIndex: 16, endColumnIndex: 17 },
          rule: { condition: { type: "ONE_OF_LIST", values: [{ userEnteredValue: "Instagram" }, { userEnteredValue: "WhatsApp" }, { userEnteredValue: "Website" }, { userEnteredValue: "Referral" }, { userEnteredValue: "Manual" }] }, showCustomUi: true, strict: false },
        },
      });
      formatRequests.push(
  ...
  if (supabase) {
    await rebuildSummarySheet(accessToken, spreadsheetId, updatedTabs, supabase);
  }

  return { created: tabsToCreate.length, tabs: tabsToCreate, summary: true };
}

// ── Push event to monthly sheet ──
...
serve(async (req) => {
  ...
  try {
    ...
    if ((action === "push_single" || action === "update_pushed") && event_id) {
      ...
      // Also rebuild summary after push
    ...
    if (action === "update_design") {
      const formatReqs: any[] = [];
      for (const tab of tabs) {
        if (tab.title.trim().toLowerCase() === SUMMARY_TAB_NAME.toLowerCase()) continue;
        formatReqs.push({
          repeatCell: {
